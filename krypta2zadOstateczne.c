...
{   FILE *fIN, *fOUT;  
  ...
  char wee[255];
   ...
   if (konf==NULL)
   {   //fclose(konf);
	 if(argc!=5) {printf("Zła liczba argumentów do stworzenia pliku konf"); return 0;}
	   ...
	   fclose(konf2);
		...
		encrypt(fIN, fOUT, "komiksyeee", "danemisasa");
     		...
     		fclose(fOUT);                      
                   ...
                   if(argc!=3) {printf("Zła liczba argumentów "); return 0;}    
		...
		char*  wektor;
           	char line[4096];
            	sprintf(line,"./krr.exe nie %s %s %s %s %s %s", argv[1], argv[2], path1, ide1,keys1,vector1);
           	system(line);
                   char line2[4096];
            	sprintf(line2,"./test.exe  %s", argv[2]);
           	system(line2);
